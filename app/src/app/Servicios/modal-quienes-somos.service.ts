...
export class ModalQuienesSomosService {
  ...
  Alert(mensaje?: string, titulo: string = 'Atencion', tipo: string = 'i') {
    const modalRef = this.ngbModal.open(ModalQuienesSomosComponent);
  ...
  BloquearPantalla() {
    ...
    if (this.contadorBloqueo === 1) {
      this.modalBloqueoRef = this.ngbModal.open(ModalQuienesSomosComponent, { backdrop: 'static', keyboard: false });
